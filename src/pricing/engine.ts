...
export function solveImpliedVol(
  ...
  if (fa * fb > 0) {
    // The standard bracket [0.01, 10.0] failed. Two causes:
    // (A) P_target > P_max (model's theoretical maximum) — no solution exists.
    // (B) P(10.0) < P_target ≤ P_max — solutions exist but both are outside the
    //     bracket because P never exceeds P_target in [0.01, 10.0].
    // Distinguish by scanning for a sign change. If found, bisect to find σ₁.
    // If no sign change, the market price is outside the achievable range → null.
    let prevSigma = a;
    let prevF = fa;
    for (let sigma = 0.05; sigma <= 10.0; sigma += 0.05) {
      const currF = f(sigma);
      if (prevF * currF <= 0) {
        // Sign change in [prevSigma, sigma]: bisect to find σ₁.
        let lo = prevSigma, hi = sigma, fLo = prevF;
        for (let k = 0; k < 50; k++) {
          const mid = (lo + hi) / 2;
          const fMid = f(mid);
    ...
    // No sign change found: P_target is outside the model's achievable range.
    return null;
  }
...
// Polymarket crypto fee formula changes on 2026-03-30:
//   Until then:  feeRate=0.25, exponent=2  → peak effective rate 1.56% at p=0.50
//   From then:   feeRate=0.072, exponent=1 → peak effective rate 1.80% at p=0.50
// Formula: fee [USDC] = C × p × feeRate × (p×(1−p))^exponent
//   Buy orders: fee deducted in shares → net shares = C × (1 − feeRate×(p×(1−p))^exp)
//   Sell orders: fee deducted in USDC.
// Note: p×(1−p) is symmetric — identical whether p is the YES or NO price.
const _POLY_FEE_CHANGE_MS = new Date('2026-03-30T00:00:00Z').getTime();

/**
 * Polymarket taker fee in USDC per share at given entry price.
 * Applies to mid/ask (taker) orders; bid (maker) orders receive a rebate
 * and are treated as zero-fee in the app.
 */
export function polyFeePerShare(price: number, nowMs: number = Date.now()): number {
  const rate = nowMs >= _POLY_FEE_CHANGE_MS ? 0.072 : 0.25;
  const exp  = nowMs >= _POLY_FEE_CHANGE_MS ? 1     : 2;
  return price * rate * Math.pow(price * (1 - price), exp);
}
...
/**
 * Implied volatility for a vanilla Black-Scholes option via Newton-Raphson.
 * Returns sigma as a fraction (e.g. 0.55 = 55%) or null if no solution found.
 * Suitable for liquid options priced away from intrinsic value; may return null
 * for deep ITM/OTM cases where vega ≈ 0.
 */
export function bsImpliedVol(
  ...
  if (tau <= 0 || marketPrice <= 0 || S <= 0 || K <= 0) return null;

  // Brenner-Subrahmanyam initial guess (good for near-ATM); clamped to sane range
  let sigma = Math.sqrt(2 * Math.PI / tau) * marketPrice / S;
  if (!isFinite(sigma) || sigma <= 0) sigma = 0.5;
  sigma = Math.max(0.01, Math.min(sigma, 5.0));

  for (let i = 0; i < 100; i++) {
    const price = bsPrice(S, K, sigma, tau, optType, r);
    const err = price - marketPrice;
    if (Math.abs(err) < 1e-7) return sigma;

    // Vega = S * φ(d1) * √τ  (r=0 simplification holds for small r)
    const sqrtTau = Math.sqrt(tau);
    const d1 = (Math.log(S / K) + (r + sigma * sigma / 2) * tau) / (sigma * sqrtTau);
    const vega = S * Math.exp(-d1 * d1 / 2) / Math.sqrt(2 * Math.PI) * sqrtTau;
    if (vega < 1e-10) break;

    sigma = Math.max(0.001, Math.min(sigma - err / vega, 10.0));
  }

  // Accept result if relative pricing error < 2%
  const relErr = Math.abs(bsPrice(S, K, sigma, tau, optType, r) - marketPrice) / (marketPrice + 1e-8);
  return relErr < 0.02 ? sigma : null;
}
...
export function computeCombinedPnlCurve(
  ...
  for (const cryptoPrice of grid) {
    ...
    // Polymarket positions — each position may have its own optionType & endDate
    for (const pos of polyPositions) {
      const iv = smile
        ? interpolateSmile(smile, Math.log(cryptoPrice / pos.strikePrice))
        : pos.impliedVol;
      const posOptionType = pos.optionType ?? optionType;
      const posTau = (snapTs != null && pos.endDate != null && pos.endDate > 0)
        ? Math.max((pos.endDate - snapTs) / YEAR_SEC, 0)
        : polyTau;
      const yesPrice = priceOptionYes(cryptoPrice, pos.strikePrice, iv, posTau, posOptionType, pos.isUpBarrier, H);
      const projectedValue = pos.side === 'YES' ? yesPrice : (1 - yesPrice);
      totalPnl += (projectedValue - pos.entryPrice) * pos.quantity - pos.entryFee;
    }
