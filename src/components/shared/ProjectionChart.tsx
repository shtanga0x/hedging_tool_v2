...
const FUTURES_GREY_DARK  = '#B0BEC5';
const FUTURES_GREY_LIGHT = '#6B7280';
...
interface ProjectionChartProps {
  ...
  // Futures overlay (grey, time-independent)
...
function CustomTooltipContent({
  ...
  return (
    <div style={{
      ...
      {/* Futures overlay */}
      {hasFuturesOverlay && !hiddenLines.has(FUTURES_LINE) && valueMap.has(FUTURES_LINE) &&
        renderRow(FUTURES_LINE, valueMap.get(FUTURES_LINE)!, lineStyles.get(FUTURES_LINE)?.color ?? '#888', lineStyles.get(FUTURES_LINE))
      }
    </div>
  );
}
...
export function ProjectionChart({
  ...
  const hasFuturesOverlay = (futuresNowCurve && futuresNowCurve.length > 0) || false;
  const futuresColor = isDark ? FUTURES_GREY_DARK : FUTURES_GREY_LIGHT;
  ...
  const chartData = useMemo(() => {
    ...
    const data = combinedCurves[0].map((point, i) => {
      ...
      // Futures overlay — skip if hidden
      if (!hiddenLines.has(FUTURES_LINE) && futuresNowCurve?.[i]) row[FUTURES_LINE] = futuresNowCurve[i].pnl;
    ...
    return data;
  }, [combinedCurves, combinedLabels, hiddenLines, polyNowCurve, polyExpiryCurve, polyAtBybitExpiryCurve, bybitNowCurve, bybitExpiryCurve, futuresNowCurve, hasPolyAtBybitExpiry]);
  ...
  const { yDomain, yTicks } = useMemo(() => {
    ...
    addPoints(futuresNowCurve, FUTURES_LINE);
  ...
  const { yDomainPct, yTicksPct } = useMemo(() => {
    ...
    const pctMin = yDomain[0] / absCost * 100;
    const pctMax = yDomain[1] / absCost * 100;
    const pctRange = pctMax - pctMin;
    const rawStep = Math.max(pctRange / 6, 1e-10);
    const magnitude = Math.pow(10, Math.floor(Math.log10(rawStep)));
    const nice = [1, 2, 2.5, 5, 10].find((n) => n * magnitude >= rawStep) ?? 10;
    const step = nice * magnitude;
    const ticks: number[] = [0];
    for (let v = step; v <= pctMax; v += step) ticks.push(Math.round(v * 100) / 100);
    for (let v = -step; v >= pctMin; v -= step) ticks.push(Math.round(v * 100) / 100);
    ticks.sort((a, b) => a - b);
    return { yDomainPct: [pctMin, pctMax] as [number, number], yTicksPct: ticks };
  ...
  const allLineLabels = useMemo(() => {
    ...
    if (hasFuturesOverlay) labels.push(FUTURES_LINE);
    return labels;
  }, [combinedLabels, hasPolyOverlay, hasPolyAtBybitExpiry, hasBybitOverlay, hasFuturesOverlay]);
  ...
  const lineStyles = useMemo(() => {
    ...
    if (hasFuturesOverlay) {
      styles.set(FUTURES_LINE, { color: futuresColor, dash: '12 6', width: 2, opacity: 0.9 });
    }
    return styles;
  }, [combinedLabels, hasPolyOverlay, hasPolyAtBybitExpiry, hasBybitOverlay, hasFuturesOverlay, futuresColor]);
  ...
  const renderTooltip = useCallback(
    ...
    (props: any) => (
      <CustomTooltipContent
        ...
        hasFuturesOverlay={hasFuturesOverlay}
    ...
    [combinedLabels, cryptoSymbol, hiddenLines, currentCryptoPrice, tooltipBg, tooltipBorder, axisColor, hasPolyOverlay, hasPolyAtBybitExpiry, hasBybitOverlay, hasFuturesOverlay, lineStyles, totalEntryCost, polyEntryCost, bybitEntryCost]
  );
  ...
  if (hasFuturesOverlay) {
    legendItems.push({ label: FUTURES_LINE, color: futuresColor, dash: '12 6', width: 2 });
  }
